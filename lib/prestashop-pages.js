...
const FOOTER_PAGE_KEYWORDS = ['envio gratis', 'privacidad'];

// Páginas que se ocultan de nuestro propio sistema (admin, footer y acceso
// directo) sin tocar ni borrar nada en PrestaShop. (api/paginas.js ya filtra
// esto antes de llegar aquí para las rutas migradas/listado; se repite aquí
// para que este módulo también sea seguro si se usa directamente.)
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    let pages = rows
      ...
      .filter(p => p.slug)
      .filter(p => !HIDDEN_PAGE_IDS.includes(String(p.id)));
