...
async function resolveItems(items) {
  ...
  return [...quantities].map(([id, qty]) => {
    ...
    const available = stock.map(row => row.data).filter(row => Number(row.id_product) === id
      && Number(row.id_product_attribute || 0) === 0 && [0,50].includes(Number(row.id_shop || 0)));
    // Mi Fiestashop is shop 50; never use another tenant's stock.
    const primary = available.find(row => Number(row.id_shop) === 50) || available.find(row => Number(row.id_shop) === 0);
