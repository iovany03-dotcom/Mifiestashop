...
const DOMAIN_ORDER = ['categorias', 'almacenes', 'empleados', 'stock', 'clientes', 'pedidos', 'carritos', 'movimientos', 'movimientos_recientes'];

async function runFullSync({ baseUrl, apiKey, supabaseUrl, serviceKey, serviceRoleKey, timeBudgetMs = 45000, domains, recentTarget }) {
  ...
  const ctx = { baseUrl, apiKey, supabaseUrl, serviceKey, serviceRoleKey, timeBudgetMs, recentTarget };
  const all = { categorias: syncCategorias, almacenes: syncAlmacenes, empleados: syncEmpleados, clientes: syncClientes, pedidos: syncPedidos, carritos: syncCarritos, stock: syncStock, movimientos: syncMovimientos, movimientos_recientes: syncMovimientosRecientes };
