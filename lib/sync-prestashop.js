...
async function syncProductos(ctx) {
  ...
  async function flush() {
    if (pending.length === 0) return;
    await sbUpsert(ctx.supabaseUrl, ctx.serviceKey, 'catalogo_productos', pending, 'id');
...
const DOMAIN_ORDER = ['categorias', 'almacenes', 'empleados', 'productos', 'stock', 'clientes', 'pedidos', 'carritos', 'movimientos', 'movimientos_recientes', 'movimientos_desde'];
