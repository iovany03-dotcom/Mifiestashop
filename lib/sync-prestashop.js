...
async function syncMovimientos(ctx) {
  ...
  while (Date.now() - startedAt < ctx.timeBudgetMs) {
    const rangeFilter = encodeURIComponent(`[${sinceId + 1},${MAX_ID}]`);
    const data = await psFetch(ctx.baseUrl, ctx.apiKey, `/api/stock_movements?display=full&filter[id]=${rangeFilter}&sort=[id_ASC]&limit=0,${PAGE_SIZE}`);
    const batch = data.stock_movements || [];
    if (batch.length === 0) { reachedEnd = true; break; }
    batch.forEach(m => {
