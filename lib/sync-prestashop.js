...
async function syncResource(ctx, domain, resource, table, transform) {
  ...
  while (Date.now() - started < ctx.timeBudgetMs) {
    ...
    const data = await psFetch(ctx.baseUrl, ctx.apiKey,
      `/api/${resource}?display=full&sort=[id_ASC]&filter[id]=${range}&limit=0,250`);
    ...
    const rows = items.map(item => ({ ...transform(item), generation }));
...
const syncDirecciones = ctx => syncResource(ctx, 'direcciones', 'addresses', 'ps_direcciones', normalizeAddress);
...
async function syncPedidos(ctx, rawBatch) {
  ...
  const rows = rawOrders.map(o => {
    ...
    return {
      ...
      payment: o.payment || '', total_paid: parseFloat(o.total_paid || 0),
