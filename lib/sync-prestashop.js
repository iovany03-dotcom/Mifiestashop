...
async function syncMovimientos(ctx) {
  ...
  // otra rama de migración. Se filtra por almacén real en vez de asumir
  // que todo lo que trae este recurso es de esta tienda.
  ...
  const warehouseFilter = encodeURIComponent(`[${warehouseIds.join('|')}]`);

  // DEBUG TEMPORAL: prueba varias variantes del filtro en una sola corrida
  // para no gastar un ciclo de deploy por cada hipótesis. Se quita en
  // cuanto se identifique cuál variante sí trae datos.
  if (ctx.debugFilters) {
    const variants = {
      soloAlmacen: `/api/stock_movements?display=full&filter[id_warehouse]=${warehouseFilter}&sort=[id_ASC]&limit=0,5`,
      almacenComillas: `/api/stock_movements?display=full&filter[id_warehouse]=${encodeURIComponent(warehouseIds.join('|'))}&sort=[id_ASC]&limit=0,5`,
      unSoloAlmacen: `/api/stock_movements?display=full&filter[id_warehouse]=53&sort=[id_ASC]&limit=0,5`,
      idYUnAlmacen: `/api/stock_movements?display=full&filter[id]=${encodeURIComponent('[1,999999999]')}&filter[id_warehouse]=53&sort=[id_ASC]&limit=0,5`,
    };
    const out = {};
    for (const [name, path] of Object.entries(variants)) {
      try {
        const data = await psFetch(ctx.baseUrl, ctx.apiKey, path);
        const arr = Array.isArray(data) ? data : (data.stock_mvts || null);
        out[name] = { path, isArray: Array.isArray(data), keys: Array.isArray(data) ? null : Object.keys(data), len: arr ? arr.length : null, raw: JSON.stringify(data).slice(0, 300) };
      } catch (e) {
        out[name] = { path, error: e.message };
      }
    }
    return { debugFilters: out };
  }
  ...
  let lastSeenId = sinceId;
  let reachedEnd = false;
  let debugFirstBatch = null;

  async function flush() {
    if (pending.length === 0) return;
    await sbUpsert(ctx.supabaseUrl, ctx.serviceKey, 'ps_inventory_movements', pending, 'id');
  ...
  while (Date.now() - startedAt < ctx.timeBudgetMs) {
    const rangeFilter = encodeURIComponent(`[${sinceId + 1},${MAX_ID}]`);
    const path = `/api/stock_movements?display=full&filter[id]=${rangeFilter}&filter[id_warehouse]=${warehouseFilter}&sort=[id_ASC]&limit=0,${PAGE_SIZE}`;
    const data = await psFetch(ctx.baseUrl, ctx.apiKey, path);
    const batch = data.stock_mvts || [];
    if (debugFirstBatch === null) debugFirstBatch = { path, warehouseIds, keys: Object.keys(data), batchLen: batch.length, raw: JSON.stringify(data).slice(0, 600) };
    if (batch.length === 0) { reachedEnd = true; break; }
    const badWarehouse = batch.find(m => !warehouseIds.includes(Number(m.id_warehouse)));
    if (badWarehouse) console.log('[sync-movimientos] ALERTA almacén fuera de rango', badWarehouse.id, badWarehouse.id_warehouse);
    batch.forEach(m => {
  ...
  return { total, debug: debugFirstBatch };
}
...
// final) antes que clientes/pedidos/carritos. "movimientos" va al final y
// aparte: es un histórico de una sola vez (no cambia hora con hora como el
// resto), así que solo corre cuando se pide explícito
// (?domain=movimientos), nunca en el cron automático — los dos crons ya
// enumeran sus dominios explícitamente en vercel.json, sin incluirlo.
const DOMAIN_ORDER = ['categorias', 'almacenes', 'empleados', 'stock', 'clientes', 'pedidos', 'carritos', 'movimientos'];

async function runFullSync({ baseUrl, apiKey, supabaseUrl, serviceKey, timeBudgetMs = 45000, domains, debugFilters }) {
  ...
  const ctx = { baseUrl, apiKey, supabaseUrl, serviceKey, timeBudgetMs, debugFilters };
  const all = { categorias: syncCategorias, almacenes: syncAlmacenes, empleados: syncEmpleados, clientes: syncClientes, pedidos: syncPedidos, carritos: syncCarritos, stock: syncStock, movimientos: syncMovimientos };
