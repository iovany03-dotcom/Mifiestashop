...
async function sbStorageUpload(supabaseUrl, serviceKey, bucketPath, bytes, contentType) {
  const r = await fetch(`${supabaseUrl}/storage/v1/object/${bucketPath}`, {
    method: 'POST',
    headers: {
      apikey: serviceKey, Authorization: `Bearer ${serviceKey}`,
  ...
  if (!r.ok) {
    const text = await r.text().catch(() => '');
    throw new Error(`storage upload ${bucketPath} -> HTTP ${r.status}: ${text.slice(0, 300)}`);
  }
}
...
// ================= CATÁLOGO DE PRODUCTOS (cursor retomable) =================
// Copia completa del catálogo (precio, activo, categoría, umbral de stock,
// etc.) hacia catalogo_productos — antes esto SOLO se leía en vivo de
// PrestaShop en cada carga de /api/productos, sin ningún respaldo propio.
// Mismo patrón de cursor retomable que syncStock: ~5000 productos no caben
// siempre en una sola corrida de function serverless.
// Precios de mayoreo reales (recurso "specific_prices", grupo 60) — mismo
// cálculo que ya usaba api/productos.js en vivo: una regla por producto con
// un umbral de piezas y una reducción sobre el precio base; se usa la regla
// con el umbral más bajo. Se trae una sola vez por corrida (no por
// producto), igual que categorías.
const WHOLESALE_GROUP_ID = '60';

async function fetchWholesaleRules(ctx) {
  try {
    const fields = '[id,id_product,id_group,from_quantity,reduction,reduction_type,price]';
    const data = await psFetch(ctx.baseUrl, ctx.apiKey, `/api/specific_prices?filter[id_group]=${WHOLESALE_GROUP_ID}&display=${encodeURIComponent(fields)}&limit=0,5000`);
...
function computeWholesaleOffer(basePrice, rule) {
  if (!rule) return null;
...
async function syncProductos(ctx) {
  // catalogo_productos solo permite SELECT con la llave anon (va a
  // controlar precio real del catálogo público, a diferencia de ps_stock/
  // ps_categorias) — escribir requiere la llave de servicio real.
  if (!ctx.serviceRoleKey) throw new Error('Falta SUPABASE_SERVICE_ROLE_KEY: catalogo_productos requiere la llave de servicio, no la anon');
  const state = await sbGetSyncState(ctx.supabaseUrl, ctx.serviceKey, 'productos');
  let sinceId = state && state.last_synced_id ? Number(state.last_synced_id) : 0;
  const MAX_ID = 999999999;
  ...
  const wholesaleRules = await fetchWholesaleRules(ctx);

  const PAGE_SIZE = 300; // productos trae mucho más texto por fila que stock (descripciones) — página más chica
  ...
  async function flush() {
    if (pending.length === 0) return;
    await sbUpsert(ctx.supabaseUrl, ctx.serviceRoleKey, 'catalogo_productos', pending, 'id');
  ...
  while (Date.now() - startedAt < ctx.timeBudgetMs) {
    const rangeFilter = encodeURIComponent(`[${sinceId + 1},${MAX_ID}]`);
    const data = await psFetch(ctx.baseUrl, ctx.apiKey, `/api/products?display=${encodeURIComponent(fields)}&filter[id]=${rangeFilter}&sort=[id_ASC]&limit=0,${PAGE_SIZE}`);
    const batch = data.products || [];
    if (batch.length === 0) { reachedEnd = true; break; }
    batch.forEach(p => {
      const id = Number(p.id);
      const imgId = p.id_default_image;
      const legacyImageUrl = imgId && imgId !== '0'
        ? `${ctx.baseUrl}/api/images/products/${id}/${imgId}?ws_key=${ctx.apiKey}`
        : null;
      const basePrice = parseFloat(p.price || 0);
      // wholesale_price es, pese al nombre en el esquema de PrestaShop, el
      // costo de compra (ver api/productos.js) — no el precio de mayoreo al
      // cliente, que sale de specific_prices (wholesaleRules, abajo).
      const offer = computeWholesaleOffer(basePrice, wholesaleRules[String(id)]);
      pending.push({
        id,
        sku: p.reference || `PS-${id}`,
        barcode: p.ean13 || null,
        name: firstLangValue(p.name, `Producto ${id}`),
        description: firstLangValue(p.description, '').replace(/<[^>]*>/g, '').trim() || null,
        description_short: firstLangValue(p.description_short, '').replace(/<[^>]*>/g, '').trim() || null,
        price: basePrice,
        costo_compra: parseFloat(p.wholesale_price || 0) || null,
        price_mayoreo: offer ? Math.round(offer.price * 100) / 100 : null,
        price_mayoreo_desde_unidades: offer ? offer.fromQty : null,
        category_id: p.id_category_default ? Number(p.id_category_default) : null,
        active: p.active === '1' || p.active === 1,
        low_stock_threshold: parseInt(p.low_stock_threshold, 10) || null,
        weight: parseFloat(p.weight || 0) || null,
        width: parseFloat(p.width || 0) || null,
        height: parseFloat(p.height || 0) || null,
        depth: parseFloat(p.depth || 0) || null,
        meta_title: firstLangValue(p.meta_title, null),
        meta_description: firstLangValue(p.meta_description, null),
        meta_keywords: firstLangValue(p.meta_keywords, null),
        link_rewrite: firstLangValue(p.link_rewrite, null),
        legacy_image_url: legacyImageUrl,
        source: 'prestashop',
  ...
  const nextCursor = reachedEnd ? 0 : lastSeenId;
  await sbSetSyncState(
    ctx.supabaseUrl, ctx.serviceKey, 'productos', nextCursor,
    reachedEnd ? `ciclo completo (${total} filas)` : `parcial, retoma en id>${nextCursor} (${total} filas)`
  );
  return total;
}

// ================= FOTOS DE PRODUCTOS (re-hospedar en Supabase Storage) =================
// Descarga cada foto que catalogo_productos todavía sirve en vivo desde
// PrestaShop (legacy_image_url, construida en syncProductos) y la vuelve a
// subir al bucket "assets" de Supabase Storage, guardando la URL propia en
// catalogo_productos.images — para que las fotos sigan funcionando aunque
// PrestaShop deje de responder. Urgente correrlo MIENTRAS PrestaShop siga
// vivo: legacy_image_url deja de servir en cuanto se apague.
// Cursor retomable por id, igual que el resto de este archivo: miles de
// productos y cada uno implica dos llamadas de red (bajar + subir), no cabe
// en una sola corrida.
async function syncFotos(ctx) {
  // Igual que syncProductos: leer/escribir catalogo_productos y subir a
  // Storage requiere la llave de servicio real, no la anon.
  if (!ctx.serviceRoleKey) throw new Error('Falta SUPABASE_SERVICE_ROLE_KEY: catalogo_productos/Storage requieren la llave de servicio, no la anon');
  const state = await sbGetSyncState(ctx.supabaseUrl, ctx.serviceKey, 'fotos');
  let sinceId = state && state.last_synced_id ? Number(state.last_synced_id) : 0;
  const PAGE_SIZE = 20; // cada fila implica bajar + subir una imagen — más lento que solo JSON
  const startedAt = Date.now();
  ...
  while (Date.now() - startedAt < ctx.timeBudgetMs) {
    const url = `${ctx.supabaseUrl}/rest/v1/catalogo_productos?select=id,legacy_image_url&id=gt.${sinceId}&images=is.null&active=eq.true&legacy_image_url=not.is.null&order=id.asc&limit=${PAGE_SIZE}`;
    const r = await fetch(url, { headers: { apikey: ctx.serviceRoleKey, Authorization: `Bearer ${ctx.serviceRoleKey}` } });
    if (!r.ok) throw new Error(`leer catalogo_productos -> HTTP ${r.status}`);
    ...
    for (const row of batch) {
      ...
      lastSeenId = Math.max(lastSeenId, id);
      try {
        const imgRes = await fetch(row.legacy_image_url);
        if (!imgRes.ok) throw new Error(`descarga HTTP ${imgRes.status}`);
        const bytes = Buffer.from(await imgRes.arrayBuffer());
        const contentType = imgRes.headers.get('content-type') || 'image/jpeg';
        const path = `assets/catalogo-productos/${id}.jpg`;
        await sbStorageUpload(ctx.supabaseUrl, ctx.serviceRoleKey, path, bytes, contentType);
        const publicUrl = `${ctx.supabaseUrl}/storage/v1/object/public/${path}`;
        await sbUpsert(ctx.supabaseUrl, ctx.serviceRoleKey, 'catalogo_productos', [{ id, images: [publicUrl] }], 'id');
        total++;
      } catch (e) {
        // No se marca esta fila como resuelta (images sigue null) — el
        // cursor sí avanza más allá de ella para no atorar al resto detrás
        // de un solo producto con foto rota; queda pendiente de un
        // reintento manual puntual si hace falta.
        failed++;
      }
      if (Date.now() - startedAt >= ctx.timeBudgetMs) break;
    }
    sinceId = lastSeenId;
    if (batch.length < PAGE_SIZE) { reachedEnd = true; break; }
  }
  ...
  const nextCursor = reachedEnd ? 0 : lastSeenId;
  await sbSetSyncState(
    ctx.supabaseUrl, ctx.serviceKey, 'fotos', nextCursor,
    reachedEnd ? `ciclo completo (${total} subidas, ${failed} fallidas)` : `parcial, retoma en id>${nextCursor} (${total} subidas, ${failed} fallidas)`
  );
  return total;
}
...
const DOMAIN_ORDER = ['categorias', 'almacenes', 'empleados', 'productos', 'fotos', 'stock', 'clientes', 'pedidos', 'carritos', 'movimientos', 'movimientos_recientes', 'movimientos_desde'];
...
async function runFullSync({ baseUrl, apiKey, supabaseUrl, serviceKey, serviceRoleKey, timeBudgetMs = 45000, domains, recentTarget, recentSince }) {
  ...
  const all = { categorias: syncCategorias, almacenes: syncAlmacenes, empleados: syncEmpleados, productos: syncProductos, fotos: syncFotos, clientes: syncClientes, pedidos: syncPedidos, carritos: syncCarritos, stock: syncStock, movimientos: syncMovimientos, movimientos_recientes: syncMovimientosRecientes, movimientos_desde: syncMovimientosDesde };
