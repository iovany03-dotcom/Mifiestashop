...
${(p.reviews.length || p.theme === 'boda') ? `<section class="reviews wrap"><div class="section-heading"><div><h2>Así celebran nuestros clientes</h2><p>Experiencias compartidas con Mi Fiesta Shop.</p></div></div>${['boda','batucada'].includes(p.theme) ? galleryHtml() : ''}<div class="review-grid">${p.reviews.map(r=>`<figure><blockquote>${esc(r.quote)}</blockquote><figcaption>${esc(r.name)}</figcaption></figure>`).join('')}</div></section>` : ''}
